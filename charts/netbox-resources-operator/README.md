...
| operatorConfig.operatorBatchingWorkerLimit | string | `""` | How many objects (CRs) are reconciled concurrently (kopf batching.worker_limit). Leave unset for unlimited (the operator's default) |
| operatorConfig.operatorExecutionMaxWorkers | int | `10` | Thread pool size for synchronous handlers (kopf execution.max_workers); caps how many reconcile bodies run at once |
